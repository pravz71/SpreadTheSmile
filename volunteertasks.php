	...
	<body>
		...
		<div class="container">
			<div id="unassigned_tasks" class="row">
				<?php  
					$sql_query = "SELECT * FROM `orders` WHERE `volunteer_id` = '$volunteer_id' ORDER BY `isDelivered`, `isPicked`";
					$results  = mysqli_query($connection,$sql_query)  or die ("Error: " . mysqli_error());
					if(mysqli_num_rows($results) === 0)
					{
						$consignment = "<div class='empty_task'><h2>Your task list is empty.</h2><h3><a href='unassignedvolunteer.php'>Click here</a> to volunteer.</h3></div>";
						echo ($consignment);
					}
					while($row = mysqli_fetch_array($results,MYSQLI_ASSOC))
					{
						...
						if ($isPicked === 'NO')
						{
							$consignment .= "<form action='assets/update_task.php' method='POST'><input type='hidden' name='order_id' value='" . $order_id . "'><button type='submit' name='cancel'>Cancel</button><button type='submit' name='picked'>Picked</button><button type='submit' name='delivered' disabled=''>Delivered</button></form></div>";
						}
						elseif ($isDelivered === 'YES' || $isDelivered === 'Y') {
							$consignment .= "</div>";
						}
						elseif ($isPicked === 'YES') {
							$consignment .= "<form action='assets/update_task.php' method='POST'><input type='hidden' name='order_id' value='" . $order_id . "'><button type='submit' name='cancel' disabled=''>Cancel</button><button type='submit' name='picked' disabled=''>Picked</button><button type='submit' name='delivered'>Delivered</button></form></div>";
						}
						echo ($consignment);
					}
